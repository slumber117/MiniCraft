...
public class FacilityUI {
    public void render(UIRenderer ui, Player player, ShaderProgram shader, int width, int height, Main main) {
        ...
        if (title.isEmpty()) title = main.furnaceOpen ? "INDUSTRIAL SMELTER" : "HIGH-EFFICIENCY COOKER";
        Vector4f titleCol = (b == Block.COOKER) ? UIPalette.TACT_BLUE : (b == Block.ALLOY_FORGE ? UIPalette.TACT_GREEN : UIPalette.TACT_ORANGE);
        ...
        ui.drawSlot(shader, cx - 36, cy + 52, slotSize, main.activeFacility.getSlot(1));
        ...
        for (int i = 0; i < 27; i++) {
            ui.drawSlot(shader, invStartX + (i%9)*(ISLOT+IGAP), invStartY + (i/9)*(ISLOT+IGAP), ISLOT, pInv[i]);
        }
